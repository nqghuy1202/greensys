...
const DocChatApp = ({ context, onClose }) => {
  ...
  // Each event is a single object: { type, conv_id, msg? } or { type, conv_id, aus_id? }.

  useEffect(() => {
    if (!CURRENT_AUS_ID) return;

    function onChatEvent(_, ev) {
      if (ev.type === 'message' && ev.msg) {
        const { msg, conv_id } = ev;
        setMessages(prev => {
          const list = prev[conv_id] || [];
          if (list.some(m => String(m.msg_id) === String(msg.msg_id))) return prev;
          return {
            ...prev,
            [conv_id]: [...list, {
              msg_id:          msg.msg_id,
              from_aus_id:     msg.from_aus_id,
              from_name:       msg.from_name || 'Unknown',
              body:            msg.body,
              reply_to_msg_id: msg.reply_to_msg_id || null,
              reply_body:      msg.reply_body || null,
              reply_from_name: msg.reply_from_name || null,
              create_date:     msg.create_date || new Date().toISOString(),
            }]
          };
        });
        setConversations(prev =>
          prev.map(c => c.conv_id === conv_id ? {
            ...c,
            last_msg_preview: msg.body,
            last_msg_time:    fmtPreviewTime(msg.create_date),
            unread_count: activeConvIdRef.current === conv_id
