...
- **Global scope**: `C:/nodejs-apex-oracle/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/nodejs-apex-oracle/.gemini/notes/`)
...
2. Glob `C:/nodejs-apex-oracle/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (C:/nodejs-apex-oracle/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/nodejs-apex-oracle/.gemini/notes/` — works in any directory
