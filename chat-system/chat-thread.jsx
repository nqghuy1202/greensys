    ...
    <div className={`msg-row ${isMine ? 'mine' : ''}`}>
      ...
      <div className="msg-col">
          ...
          <div className="msg-bubble">
            ...
            {m.attach && m.attach.type !== 'img' && (
              <div className="msg-attach">
                ...
                <button type="button"className="icon-btn" style={{ width: 28, height: 28 }}><Icons.ExtLink size={14} /></button>
              </div>
            )}
          </div>
        )}
        ...
        <div className="msg-hover-actions">
          <button type="button"className="msg-hover-action" onClick={() => onReact && onReact(m, '❤️')} title="Thả cảm xúc"><Icons.Heart size={14} /></button>
          <button type="button"className="msg-hover-action" onClick={() => onReply && onReply(m)} title="Trả lời"><Icons.Reply size={14} /></button>
          <button type="button"className="msg-hover-action" onClick={() => onPin && onPin(m)} title="Ghim"><Icons.Pin size={14} /></button>
          {isMine && <button type="button"className="msg-hover-action" title="Sửa"><Icons.Edit size={14} /></button>}
          {isMine && <button type="button"className="msg-hover-action danger" title="Thu hồi"><Icons.Trash size={14} /></button>}
          <button type="button"className="msg-hover-action" title="Thêm"><Icons.More size={14} /></button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="chat-head">
      ...
      <div className="chat-head-actions">
        ...
        <button type="button"className="icon-btn" title="Thêm thành viên"><Icons.Users size={16} /></button>
        <button type="button"className={`icon-btn ${infoOpen ? 'active' : ''}`} title="Thông tin chứng từ" onClick={onToggleInfo}><Icons.PanelR size={16} /></button>
        ...
        <button type="button"className="icon-btn" title="Đóng" onClick={onClose}><Icons.Close size={16} /></button>
      </div>
    </div>
  );
};
    ...
    <div className="composer-wrap" style={{ position: 'relative' }}>
      ...
      {replyingTo && (
        <div className="composer-reply-banner">
          ...
          <button type="button"className="icon-btn" style={{ width: 24, height: 24 }} onClick={onCancelReply}><Icons.X size={12} /></button>
        </div>
      )}
      ...
      <div className={`composer ${replyingTo ? 'with-reply' : ''}`}>
        ...
        <div className="composer-bottom">
          <button type="button"className="icon-btn" title="Đính kèm file"><Icons.Paperclip size={16} /></button>
          <button type="button"className="icon-btn" title="Gửi ảnh"><Icons.Image size={16} /></button>
          <button type="button"className="icon-btn" title="Nhắc người"><Icons.At size={16} /></button>
          <button type="button"className="icon-btn" title="Tag chứng từ"><Icons.Hash size={16} /></button>
          <button type="button"className="icon-btn" title="Emoji"><Icons.Smile size={16} /></button>
...
const ChatThread = ({ convo, onToggleInfo, infoOpen, onClose, showTypingDemo, hideHeader, messages: msgsProp, onSend: onSendProp }) => {
  const Icons = window.Icons;
  const baseMessages = msgsProp !== undefined ? msgsProp : (window.CHAT_DATA?.MESSAGES?.[convo.id] || []);
  const [extra, setExtra] = useState([]);
  const [replyingTo, setReplyingTo] = useState(null);
  const messagesRef = useRef(null);

  const messages = msgsProp !== undefined ? msgsProp : [...baseMessages, ...extra];
  ...
  const handleSend = (text) => {
    if (onSendProp) {
      onSendProp(text, replyingTo?.id || null);
      setReplyingTo(null);
      return;
    }
    const newMsg = {
  ...
  return (
    <div className="chat-pane">
      {!hideHeader && <ChatHeader convo={convo} onToggleInfo={onToggleInfo} infoOpen={infoOpen} onClose={onClose} />}
      ...
      {pinned && pinned.attach && (
        <div className="chat-pinned">
          ...
          <button type="button"className="icon-btn" style={{ width: 24, height: 24 }}><Icons.ChevDown size={14} /></button>
        </div>
      )}
