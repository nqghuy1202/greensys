apex.jQuery( apex.gPageContext$ ).on( "apexreadyend", function( e ) {
    $('.s-g-container').each(function(){
        kanban.initialize('#'+this.id, this.getAttribute('dropitemajax'), this.getAttribute('dropgroupajax'), this.getAttribute('columnsajax'))
    });
} );
...
var kanban = (function ($) {
    ...
    return {
        initialize: function ( parentID, dropitemajax, dropgroupajax, columnsajax) {
            ...
            function buildDynamicColumns(columns) {
                ...
                // Header row = first .s-g-row.kb-row inside parent (the column header strip)
                var $headerRow = parent.find('.s-g-row.kb-row').first();

                // Remove previously injected dynamic content to avoid duplicates on page change
                $headerRow.find('.kb-col[data-dynamic]').remove();
                parent.find('.kb-col.kb-item-region[data-dynamic]').remove();

                // Inject column header divs
                columns.forEach(function(col) {
                    var $header = $('<div class="kb-col kb-col-header" data-dynamic="1">')
                        .attr('parentheaderid', col.id)
                        .attr('card-icon-color', col.color || '#4A90D9');
                    var $headerContent = $('<div class="kb-col-header-content">')
                        .css('border-top-color', col.color || '#4A90D9');
                    $headerContent.append($('<p class="title">').text(col.text));
                    $header.append($headerContent);
                    $headerRow.append($header);
                });

                // Inject item containers into every group row (.reportgroup.kb-row)
                parent.find('.reportgroup.kb-row').each(function() {
                    var $groupRow = $(this);
                    columns.forEach(function(col) {
                        var $region = $('<div class="kb-col kb-item-region" data-dynamic="1">');
                        var $content = $('<div class="kb-col-content">').css('border-left-color', col.color || '#4A90D9');
                        var $container = $('<div class="kb-item-container" data-dynamic="1">').attr('datastatus', col.id);
                        $content.append($container);
                        $region.append($content);
                        $groupRow.append($region);
                    });
                });
            }
            ...
            if (columnsajax) {
                apex.server.process(columnsajax, {}, {
                    success: function(data) {
                        buildDynamicColumns(data.columns);
                        startKanban();
                    },
                    error: function() {
                        startKanban();
            ...
            function reinitDragula() {
                if (columnsajax) {
                    apex.server.process(columnsajax, {}, {
                        success: function(d) {
                            buildDynamicColumns(d.columns);
                            relocate();
                            if (drakeItems) { drakeItems.destroy(); }
                            if (drakeGroups) { drakeGroups.destroy(); }
                            addDragulaToItems();
                            addDragulaToGroups();
                        },
                        error: function() {
            ...
            $(parentID).on( "apexafterrefresh", function() {
                reinitDragula();
            } );
            ...
            function addDragulaToItems() {
                ...
                drakeItems = dragula($(parent.find('.kb-item-container')).toArray(), {
                    ...
                    mirrorContainer: document.querySelector(parentID)
                });
                lastItemsData = [];
                ...
                drakeItems.on("drag", function (el, source) {
                    ...
                    if(cardData.length == 0){
                        parent.trigger('kanbanmanualalterationdetected', [dragData]);
                        return;
                    }
                ...
                drakeItems.on("drop", function (el, target, source, sibling) {
                    ...
                    if(cardData.length == 0){
                        parent.trigger('kanbanmanualalterationdetected', [dropData]);
                        return;
                    }
                            ...
                            apex.server.process(dropitemajax,
                                {
                                    ...
                                    success: function (d) {
                                        if(d.success){
                                            ...
                                            GreenSysMessage("error", d.sqlcode)
            ...
            function addDragulaToGroups() {
                ...
                lastGroupsData = [];
                $(parent).find('.reportgroup').each(function() {
                    var groupid = $(this).attr('data-group-id');
                    if (groupid && !lastGroupsData?.some(group => group.groupid === groupid)) {
                        lastGroupsData.push({
                            "groupid":groupid
                        });
                    }
                ...
                drakeGroups.on("drag", function (el, source) {
                    ...
                    var groupData = lastGroupsData.filter(function(e) {
                        return e.groupid == dragData.groupId
                    });
                ...
                drakeGroups.on("drop", function (el, target, source, sibling) {
                    ...
                    var groupData = lastGroupsData.filter(function(e) {
                        return e.groupid == dropData.groupId
                    });
