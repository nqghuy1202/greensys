      ...
      <div className="lp-header">
        <div className="lp-header-row">
          ...
          <div className="lp-header-actions" ref={createRef}>
            <button type="button"className={`lp-action-btn ${createOpen ? 'active' : ''}`} title="Tạo nhóm hoặc soạn tin mới" onClick={(e) => { e.stopPropagation(); setCreateOpen(o => !o); }}>
              <Icons.Plus size={18} />
            </button>
          </div>
        </div>
      ...
      {statusOpen && (
        <div className="lp-status-popover" onClick={e => e.stopPropagation()}>
          ...
          <div className="status-custom-input">
            ...
            />
            <button type="button"className="icon-btn" style={{ background: 'var(--primary)', color: 'white', width: 30, height: 30 }} onClick={() => { onSetStatusText(customText); setStatusOpen(false); }}>
              <Icons.Check size={14} />
            </button>
          </div>
        </div>
