...
const PageApp = () => {
  ...
  useEffectPageApp(() => {
    ...
    async function poll() {
      if (cancelled) return;
      try {
        const data = await apexCall('chatEvents');
        if (cancelled) return;
        if (data.type === 'message' && data.message) {
          const cid = String(data.conv_id);
          const msg = normalizeOneMessage(data.message, MY_AUS_ID);
          // Chỉ append nếu conv đã được load (tránh phá cache)
