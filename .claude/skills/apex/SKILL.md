...
- `$SQLCL_CONNECTION` = full connection string, e.g. `dev24/"password"@jdbc:oracle:thin:@//host:1521/service`
- Always end with `exit;` to ensure the process terminates
- Use heredoc (`<<'EOF'`) to pass multi-line commands
- For single commands: `sql -S $SQLCL_CONNECTION <<< "SELECT 1 FROM dual; exit;"`
